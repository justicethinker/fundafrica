...
var FieldElem51 = /** @class */ (function () {
...
var MontgomeryPoint = /** @class */ (function () {
    ...
    */
    MontgomeryPoint.prototype.to_edwards = function (sign_bit) {
        ...
        if (u.ct_eq(FieldElem51.minus_one()))
            return undefined;
        var one = FieldElem51.one();
...
function straus_optional_multiscalar_mul(scalars, points) {
    var nafs = scalars.map(function (s) { return scalar_non_adjacent_form(s, 5); });
    var lookupTables = points.map(function (p) { return NafLookupTable.fromEdwardsPoint(p); });
...
function pippenger_optional_multiscalar_mul(scalars, points) {
    ...
    var scalras_digits = scalars.map(function (s) { return scalar_to_radix_2w(s, w); });
    var points_proj = points.map(function (p) { return p.toProjectiveNiels(); });
    // zipped
    var scalars_points = scalras_digits.map(function (s, i) { return [s, points_proj[i]]; });
    ...
    var columns = new Array(buckets_count).fill(0).map(function () { return EdwardsPoint.IDENTITY; });
    for (var i = digits_count - 1; i >= 0; i--) {
        for (var j = 0; j < buckets_count; j++)
    ...
    return columns.reduce(function (total, p) { return total.mulByPow2(w).add(p); });
}
...
function scalar_to_radix_2w(scalar, w) {
    if (w === 4)
        return scalar_to_radix_16(scalar);
...
var EdwardsPoint = /** @class */ (function () {
...
pub fn mul(a: &Scalar, A: &EdwardsPoint, b: &Scalar) -> EdwardsPoint {
    ...
    */
    EdwardsPoint.prototype.mul_by_pow_2 = function (k) {
        if (k <= 0)
            throw new Error("mul_by_pow_2 :: k must be greater than 0");
        var r;
        var s = this.toProjective();
        for (var i = 0; i < k - 1; i++) {
            r = s.double();
...
var NafLookupTable = /** @class */ (function () {
    ...
    */
    NafLookupTable.fromEdwardsPoint = function (point) {
        var Ai = new Array(8).fill(0).map(function (_) { return point.toProjectiveNiels(); });
        var A2 = point.double();
        for (var i = 0; i < 7; i++) {
...
var AffineNielsPoint = /** @class */ (function () {
...
*/
var ProjectiveNielsPoint = /** @class */ (function () {
    ...
    ProjectiveNielsPoint.prototype.compress = function () {
    ...
    ProjectiveNielsPoint.prototype.neg = function () {
        ...
        this.Y_minus_X.clone(), this.Y_plus_X.clone(), 
        // same Z
        this.Z.clone(), 
        // negate T2d
        this.T2d.neg());
    ...
    */
    ProjectiveNielsPoint.tableFromEdwardPoint = function (point) {
        var points = new Array(LookupTableProjectiveNielsPoint.SIZE).fill(0).map(function (_) { return point.toProjectiveNiels(); });
...
*/
var LookupTableProjectiveNielsPoint = /** @class */ (function () {
    ...
    */
    ...
    LookupTableProjectiveNielsPoint.SIZE_RANGE = [1, 2, 3, 4, 5, 6, 7, 8];
    LookupTableProjectiveNielsPoint.SIZE_RANGE_MIN = 1;
    LookupTableProjectiveNielsPoint.SIZE_RANGE_MAX = 8;
    LookupTableProjectiveNielsPoint.CONVERSION_RANGE = [0, 1, 2, 3, 4, 5, 6, 7];
...
var CompletedPoint = /** @class */ (function () {
...
var ProjectivePoint = /** @class */ (function () {
...
function read_u64_into_LE(src, dst, dst_len) {
    if (dst_len === void 0) { dst_len = (Math.min(dst.length * 8, src.length) / 8) >>> 0; }
